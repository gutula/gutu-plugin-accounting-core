...
import { businessFlowDefinitions, postBillingDocument, allocatePayment, closeAccountingPeriod, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("accounting-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("accounting.billing.post");
    expect(manifest.publicEvents).toContain("accounting.billing-posted.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof postBillingDocument).toBe("function");
    expect(typeof allocatePayment).toBe("function");
    expect(typeof closeAccountingPeriod).toBe("function");
