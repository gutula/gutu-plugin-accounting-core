export type AccountingCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildAccountingCoreSqliteMigrationSql(options: AccountingCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "accounting_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getAccountingCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getAccountingCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildAccountingCoreSqliteRollbackSql(options: AccountingCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "accounting_core_");
  return [
...
export function getAccountingCoreSqliteLookupIndexName(tablePrefix = "accounting_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getAccountingCoreSqliteStatusIndexName(tablePrefix = "accounting_core_"): string {
